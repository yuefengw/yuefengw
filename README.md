...
**核心技术：** 
- 基于 MinIO 搭建对象存储承载大文件上传，使用 Kafka 构建文档异步处理链路，解耦上传、DeepDoc 解析、切块与向量化，并将向量及关键词存入 Elasticsearch 支持混合检索。
- 优化 Chunk 切分策略，设计基于文档结构识别、递归细切、表格表头保留、列表前导句合并的切分流程，并通过 2000 条 QA 测试集完成量化验证，知识库检索召回率由 67% 提升至 91%。
