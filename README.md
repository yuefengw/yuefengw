...
### **GeekSeek - RAG问答系统开发与优化**

**项目描述：** GeekSeek是一个基于私有知识库的企业级智能对话平台，允许用户上传文档构建专属知识空间，并通过自然语言交互方式查询和获取知识。它结合了大语言模型和向量检索技术，能够让用户能够通过对话的形式与自己的知识库进行高效交互。

**技术栈：** Spring Boot、SpringAI、MySQL、Redis、Kafka、Elasticsearch、MinIO、WebSocket、DeepDoc

**核心技术：** 
- 基于 MinIO 搭建对象存储承载大文件上传，使用 Kafka 构建文档异步处理链路，解耦上传、DeepDoc 解析、切块与向量化，并将解析结果与索引数据写入 Elasticsearch 支持混合检索。
- 优化文档切块与索引方案，设计基于文档结构识别、递归细切、表格表头保留、列表前导句合并的切分流程，并结合文档层级树与动态粒度检索减少语义链断裂；通过 2000 条 QA 测试集验证，知识库检索召回率由 67% 提升至 91%，F1 提升约 10%。
- 基于规则、BERT 分类器和 Prompt Engineering 实现查询意图识别，对口语化、错别字及短 Query 进行重写与扩写，并结合 HyDE 在召回前生成假设文档增强语义表示，提升检索命中率与复杂查询召回效果。
- 基于 Elasticsearch + IK 分词器构建多格式文档索引，结合通义千问 Embedding 完成 2048 维向量化，融合 BM25 关键词召回与 KNN 语义召回，并结合 RRF 融合与 Cross-Encoder 二阶段重排优化候选排序，支持关键词与语义混合检索，多轮测试下MRR、NDCG均有20%+提升。
- 引入 MCP 协议封装本地文件操作、PDF 生成与数据库查询等工具能力，参考 OpenManus 设计分层智能体架构，并基于 SpringAI FunctionCallback 实现工具调用与 ReAct 推理，支持检索、生成、总结 PDF 等多步骤任务编排。
- 基于 Redis 持久化多轮会话历史，并通过摘要记忆策略压缩超长上下文，降低推理成本；同时基于 WebSocket 与大模型 Stream API 实现流式响应，提升问答实时性与用户体验。
- 构建 RAG 效果评测与模型对比机制，设计多层次文档匹配算法评估检索质量，针对 bge、qwen 等向量模型输出 Precision、Recall、F1、MRR 等指标，为检索与召回优化提供量化依据。
